...
class PurchaseRequestLineMakePurchaseOrderGainde(models.TransientModel):
    _name = "purchase.request.line.make.purchase.order.gainde"
    _description = "Purchase Request Line Make Purchase Order"

    supplier_id = fields.Many2one(
        comodel_name="res.partner",
        string="Supplier",
        required=False,  # <<< n'est plus obligatoire
        context={"res_partner_search_mode": "supplier"},
    )
     # --- Montant / devise pour le wizard ---
    ...
    item_ids = fields.One2many(
        comodel_name="purchase.request.line.make.purchase.order.item.gainde",
        inverse_name="wiz_id",
        string="Items",
    )
    ...
    def _compute_estimated_total(self):
        for item in self:
            item.estimated_total = (item.estimated_price or 0.0) * (
    ...
    def _prepare_item(self, line):
        return {
            ...
            "preferred_vendor_id": getattr(line, "preferred_vendor_id", False)
            and line.preferred_vendor_id.id
            or False,
        }
    ...
    def _check_valid_request_line(self, request_line_ids):
        ...
        for line in self.env["purchase.request.line"].browse(request_line_ids):
            ...
            #raise UserError()  # --- IGNORE ---
            # if line.request_id.state not in ["to_purchase", "in_progress"]:
            #     raise UserError(
    ...
    @api.model
    def _prepare_purchase_order(self, picking_type, group_id, company, origin,department_id,description):
        """Prépare le dictionnaire pour le PO.
        Si supplier_id est vide, on crée un PO sans partenaire.
        """
        ...
        # sinon pas de partner_id → PO sans fournisseur
        return data
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        return {
            ...
            # Utilise le prix de la ligne de demande d'achat originale
            "price_unit": item.line_id.estimated_price or 0.0,
            "product_qty": qty,
    ...
    def _get_purchase_line_name(self, order, line):
        """Fetch the product name as per supplier settings (si fournisseur)"""
    ...
    def make_purchase_order_grouped_by_preferred_vendor(self):
        """
        Variante de make_purchase_order :
        - un bon de commande par fournisseur préféré (preferred_vendor_id),
        - un bon de commande sans fournisseur pour les lignes sans preferred_vendor_id.
        """
        self.ensure_one()
        res = []
        purchase_obj = self.env["purchase.order"]
        ...
        # clé = vendor_id (ou False) -> purchase.order
        po_by_vendor = {}
        ...
        for item in self.item_ids:
            ...
            vendor = item.preferred_vendor_id  # peut être vide
            vendor_id = vendor.id or False

            # 1) Récupérer / créer le bon de commande pour ce fournisseur
            purchase = po_by_vendor.get(vendor_id)
            if not purchase:
                po_vals = {
                    "origin": line.origin,
                    ...
                    'department_id': line.request_id.department_id.id,
                }
                if vendor:
                    po_vals.update(
                        {
                            ...
                            if vendor.property_account_position_id
                            else False,
                        }
                    )
                # si vendor est False, on crée un PO sans partner_id
                purchase = purchase_obj.create(po_vals)
                po_by_vendor[vendor_id] = purchase

            # 2) Fusion des lignes dans CE bon de commande (par produit / UoM / analytique)
            product = item.product_id
            if not product:
                raise UserError(_("Please select a product for all lines"))

            # UoM allocation = UoM de la ligne de PR
            if not line.product_uom_id:
                line.product_uom_id = item.product_uom_id
            alloc_uom = line.product_uom_id
            wizard_uom = item.product_uom_id

            # Domaine de recherche pour fusionner sur la même ligne de PO
            # (même commande, produit, UoM, analytique, date_planned éventuellement)
            product_uom = product.uom_po_id.id or product.uom_id.id
            domain = [
            ...
            if available_po_lines and not item.keep_description:
                # 2.a) On réutilise une ligne existante
                new_pr_line = False
                po_line = available_po_lines[0]
                po_line.purchase_request_lines = [(4, line.id)]
                po_line.move_dest_ids |= line.move_dest_ids
                ...
                # 2.b) On crée une nouvelle ligne de PO
                # quantité dans l'UoM d'achat
                qty = wizard_uom._compute_quantity(
                    item.product_qty, product.uom_po_id or product.uom_id
                )
                # min_qty par fournisseur si disponible
                if vendor:
                    min_qty = line._get_supplier_min_qty(product, vendor)
                    qty = max(qty, min_qty)
                ...
                po_line_vals = {
                    ...
                    "date_planned": self._get_date_with_user_tz(date_required),
                    "move_dest_ids": [(4, x.id) for x in line.move_dest_ids],
                }

                # libellé
                if item.keep_description and item.name:
                    name = item.name
                else:
                    name = product.display_name
                    if product.description_purchase:
                        name += "\n" + product.description_purchase
                po_line_vals["name"] = name
            ...
            # 3) Recalcul de la quantité restante sur la ligne de PR
            new_qty = pr_line_obj._calc_new_qty(
                line, po_line=po_line, new_pr_line=new_pr_line
            )
        ...
        # Mettre les demandes en "in_progress"
        #purchase_requests = self.item_ids.mapped("request_id")
        #purchase_requests.button_in_progress()

        # enlever les doublons dans res
        res = list(set(res))
...
class PurchaseRequestLineMakePurchaseOrderItemGainde(models.TransientModel):
    _name = "purchase.request.line.make.purchase.order.item.gainde"
    _description = "Purchase Request Line Make Purchase Order Item"

    wiz_id = fields.Many2one(
        comodel_name="purchase.request.line.make.purchase.order.gainde",
        string="Wizard",
        required=True,
        ondelete="cascade",
        readonly=True,
    ...
    line_id = fields.Many2one(
        comodel_name="purchase.request.line", string="Purchase Request Line",    domain=[("state", "=", "approved")]
    ...
    estimated_price = fields.Monetary(
    string="Estimated Unit Price",
    related="line_id.estimated_price",
    currency_field="currency_id",
    readonly=True,
    )
    ...
    def _compute_estimated_total(self):
      for rec in self:
        rec.estimated_total = (rec.estimated_price or 0.0) * (rec.product_qty or 0.0)
...
class PurchaseRequestRejectWizard(models.TransientModel):
    ...
    _description = 'Wizard de rejet de demande d’achat'

    request_ids = fields.Many2many('purchase.request', string='Demandes')
    motif = fields.Text(string='Motif du rejet', required=True)

    @api.model
    def default_get(self, fields_list):
        res = super().default_get(fields_list)
        active_ids = self.env.context.get('default_request_ids') or self.env.context.get('active_ids') or []
        if active_ids:
    ...
    def action_confirm(self):
        ...
        for req in self.request_ids:
            # Enregistre le motif puis rejette
            vals = {'state': 'rejected'}
            if 'x_reject_motif' in req._fields:
                vals['x_reject_motif'] = self.motif
            req.write(vals)
            req.message_post(body=_("Rejeté. Motif: %s") % (self.motif,))
