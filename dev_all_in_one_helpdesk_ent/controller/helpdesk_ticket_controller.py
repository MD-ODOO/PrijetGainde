...
class CustomerPortal(portal.CustomerPortal):
    def _prepare_my_tickets_values(self, page=1, date_begin=None, date_end=None, sortby=None, filterby='all', search=None, groupby='none', search_in='content'):
        ...
        if search and search_in:
            search_domain = []
            if search_in in ('ticket_ref'):
                search_domain =   [('ticket_ref', 'ilike', search)]  #OR([search_domain, [('name', 'ilike', search)]])
            if search_in in ('content'):
                search_domain = ['|', ('name', 'ilike', search), ('description', 'ilike', search)]
            if search_in in ('user'):
                search_domain = [('user_id', 'ilike', search)]
            if search_in in ('message'):
                search_domain = [('message_ids.body', 'ilike', search),('message_ids.subtype_id', '=', discussion_subtype_id)]
...
#        ======automatic load running month Start nd End date=====
        today = fields.Date.today()
        start_date= datetime.date(today.year, today.month, 1)
        end_date = datetime.date(today.year, today.month, calendar.mdays[today.month])
...
class PortalHelpdeskTicketExport(http.Controller):    
    @http.route('/my/ticket/export', type='http', auth='public', website=True, csrf=False)
    def portal_timesheet_export(self, start_date=None, end_date=None, **kwargs):
        print("s==",start_date)
        print("e==",end_date)

        if not start_date or not end_date:
            return request.redirect('/my/ticket')  # Redirect to a relevant page if dates are not provided
        ...
        wizard = request.env['helpdesk.ticket.excel.wizard.portal'].create(values)
        action = wizard.helpdesk_ticket_excel()
