...
class ProjectTaskPortal(portal.CustomerPortal):
    def _check_project_access_rights(self):
        """
        Custom method to restrict access to Project Tasks portal pages.
        Redirects unauthorized users to /my or 404.
        """
        if not request.env.user.has_group("pk_advance_employee_portal.ad_group_portal_tasks"):
    ...
    def portal_project_tasks_view(self, page=1, **kw):
        ...
        pager_data = pager(url="/tasks/my", total=task_count, page=page,step=10)

        tasks = Task.search(domain, limit=10, offset=pager_data['offset'])
    ...
    def portal_task_view(self, task_id, **kw):
        ...
        # ✅ Limit projects to those where user has assigned tasks
        assigned_tasks = request.env['project.task'].sudo().search([('user_ids', 'in', user.id)])
        allowed_project_ids = assigned_tasks.mapped('project_id').ids
        allowed_projects = request.env['project.project'].sudo().search([('id', 'in', allowed_project_ids)])

        # ✅ Limit milestones to those in allowed projects
        allowed_milestones = request.env['project.milestone'].sudo().search([('project_id', 'in', allowed_project_ids)])

        # ✅ Limit stages to those in allowed projects
        allowed_stages = request.env['project.task.type'].sudo().search([('project_ids', 'in', allowed_project_ids)])

        # ✅ Limit users to only users assigned to tasks of this employee
        allowed_user_ids = assigned_tasks.mapped('user_ids').ids
        allowed_users = request.env['res.users'].sudo().browse(allowed_user_ids)
        ...
        values = {
            ...
            'users': allowed_users,
            'partner': request.env['res.partner'].sudo().search([]),  # Partners can remain unfiltered if needed
        }
