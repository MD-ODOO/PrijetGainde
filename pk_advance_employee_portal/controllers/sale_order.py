...
class SaleOrderPortal(portal.CustomerPortal):
    def _check_so_access_rights(self):
        """
        Custom method to restrict access to Sale Order portal pages.
        Redirects unauthorized users to /my or 404.
        """
        if not request.env.user.has_group("pk_advance_employee_portal.ad_group_portal_sales"):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if request.env.user.has_group("pk_advance_employee_portal.ad_group_portal_sales"):
            if "sale_order_checkout_count" in counters:
                ...
                values["sale_order_checkout_count"] = count or 1
        return values
    ...
    def portal_sale_order_view(self, page=1, date_start=None, date_end=None, state=None, **kw):
        restrict = self._check_so_access_rights()
        if restrict:
            return restrict
