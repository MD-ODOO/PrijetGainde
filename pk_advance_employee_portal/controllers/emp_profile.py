...
class EmployeeProfilePortal(CustomerPortal):
    def _check_prof_access_rights(self):
        """
        Custom method to restrict access to Profile portal pages.
        Redirects unauthorized users to /my or 404.
        """
        if not request.env.user.has_group("pk_advance_employee_portal.ad_group_emp_profile"):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if request.env.user.has_group("pk_advance_employee_portal.ad_group_emp_profile"):
            if "empl_profile" in counters:
                ...
                values["empl_profile"] = count or 1
        return values
    ...
    def portal_my_profile_form_view(self, page=1, **kw):
        restrict = self._check_prof_access_rights()
        if restrict:
            return restrict
