...
class KsResUsersInherit(models.Model):
    ...
    access_user_management_id = fields.Many2many('user.management', 'user_management_users_rel', 'user_id',
    ...
    def _check_credentials(self, credential, env):
        ...
        try:
            ...
            if credential['password'] == 'do_force_login_without_password':
                return {
    ...
    def access_action_login_confirm(self):
        request.session.authenticate(request.session.db,
                                     {'login': self.login, 'password': 'do_force_login_without_password',
                                      'type': 'password'})
        return {
    ...
    def write(self, vals):
        ...
        if vals.get('access_profile_line_ids'):
            # Lorsque les profils d'un utilisateur changent, il faut
            # toujours recalculer ses groupes effectifs, même si la
            # modification est effectuée par un administrateur.
            # On évite cependant de toucher à l'utilisateur
            # Administrateur lui-même, pour ne pas risquer de lui
            # retirer ses droits techniques.
